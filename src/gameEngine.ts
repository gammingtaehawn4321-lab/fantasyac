...
import { EquipmentSlot, EquippedItems, EQUIPMENT_DATABASE } from './data/equipment';
...
export function createNewPlayerState(
  ...
  return {
    ...
    talentPoints: 0,
    learnedTalents: {},
    learnedSkills: ['basic_attack', 'defend_stance', 'first_aid'],
    activeBattle: null,
...
export function craftRecipe(
  ...
  if (profLvl < recipe.requiredLevel) {
    return {
      nextState: state,
      success: false,
      message: `[${PROFESSIONS_DATABASE[recipe.professionId]?.name || recipe.professionId}] 직업 레벨이 부족합니다. (필요: Lv.${recipe.requiredLevel}, 현재: Lv.${profLvl})`,
    };
  }
  ...
  try {
    ...
    if (parsed && parsed.playerState && Array.isArray(parsed.messages)) {
      ...
      const migratedPlayer: PlayerState = {
        ...
        learnedTalents,
        learnedSkills: Array.isArray(p.learnedSkills) && p.learnedSkills.length > 0
          ? p.learnedSkills
          : ['basic_attack', 'defend_stance', 'first_aid'],
      ...
      const normalizedPlayer = recalculateAdultDerivedStatus(migratedPlayer);
